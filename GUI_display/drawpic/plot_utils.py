...
class DrawPlot:
    ...
    def draw_success_rate(self):
        ...
        # 定义公式映射
        formulas = {
             '公式1: (复杂度=14) -0.0726*x1*log(x3+1)+1.1472/(x4+1.3932)': lambda x: -0.0726 * x[:, 0] * np.log(x[:, 2] + 1) + 1.1472 / (x[:, 3] + 1.3932),
             '公式2: (复杂度=10) 0.6857*log(x7)+11.4144/(x3+11.8131)': lambda x: 0.6857 * np.log(x[:, 6]) + 11.4144 / (x[:, 2] + 11.8131),
             '公式3: (复杂度=11) -0.1199*exp(x3)+1.5821*log(x7+1)': lambda x: -0.1199 * np.exp(x[:, 2]) + 1.5821 * np.log(x[:, 6] + 1),
             '公式4: (复杂度=7) 0.0028*x6+0.9934*x7': lambda x: 0.0028 * x[:, 5] + 0.9934 * x[:, 6],
             '公式5: (复杂度=9) (0.0003)*(x1+x2)+0.9815*x7': lambda x: (0.0003) * (x[:, 0] + x[:, 1]) + 0.9815 * x[:, 6],
             '公式6: (复杂度=8) 0.2562*log(x2)+-0.8782*x3': lambda x: 0.2562 * np.log(x[:, 1]) + -0.8782 * x[:, 2],
             '公式7: (复杂度=12) 1.0290*exp(-x6)+0.0198*x2*x4': lambda x: 1.0290 * np.exp(-x[:, 5]) + 0.0198 * x[:, 1] * x[:, 3],
             '公式8: (复杂度=11) 0.4525/(x6+1)+0.8490*x7**2': lambda x: 0.4525 / (x[:, 5] + 1) + 0.8490 * (x[:, 6] ** 2),
        }
